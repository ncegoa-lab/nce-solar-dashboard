...
APP_VERSION = "2026-07-20-selected-graph-clarity-v80"
...
rowsEl.querySelectorAll('tr[data-id]').forEach(tr=>{if(tr.dataset.id===activePlantId)tr.classList.add('open');tr.onclick=()=>{const nextId=tr.dataset.id===activePlantId?null:tr.dataset.id;if(nextId&&nextId!==activePlantId){productionDate=todayText();productionDatePickEl.value=productionDate;setProductionMode('day');activeTodayChartKey=''}activePlantId=nextId;render()}});
