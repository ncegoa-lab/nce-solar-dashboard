...
APP_VERSION = "2026-07-19-hide-stale-on-open-v71"
...
function staleOnlineList(list){return (list||[]).filter(p=>!fresh(p)&&!offline(p))}
...
searchInput.oninput=render;brandFilter.onchange=render;statusFilter.onchange=render;productionDate=todayText();productionDatePickEl.value=productionDate;setupDateSelectors();const boot=window.__BOOTSTRAP__||{};if(boot.status)applyStatus(boot.status);if(Array.isArray(boot.plants)){const bootStale=staleOnlineList(boot.plants);if(boot.status?.config?.auto_refresh_on_open&&bootStale.length){showOpeningRefreshPlaceholder(bootStale.length)}else{applyPlants(boot.plants,{preserveSelection:false})}}startAutoRefresh();load({preserveSelection:true}).catch(error=>{setWarning('App load failed. Showing last loaded data. '+error.message);logEl.textContent='App load failed: '+error;});
</script>
</body></html>"""
