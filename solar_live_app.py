...
data from Supabase or existing project files, can refresh cloud data, and can generate
...
def fetch_from_supabase() -> pd.DataFrame | None:
    """Queries solar data directly from Supabase DB table if environment keys exist."""
    url = os.environ.get("SUPABASE_URL")
    key = os.environ.get("SUPABASE_KEY")
    table = os.environ.get("SUPABASE_TABLE", "solar_data")
    ...
    try:
        ...
        print(f"Supabase query failed, falling back to local files: {exc}")
    return None
...
class SolarLiveApp:
    ...
    def plant_dataframe(self) -> pd.DataFrame:
        # Step 5 Logic: Try querying Supabase first, fallback to local dataset if offline or empty
    ...
    def refresh(self) -> dict[str, Any]:
        with self.refresh_lock:
            ...
            # Sync local outputs to Supabase DB if script exists
            if (PROJECT_DIR / "sync_to_supabase.py").exists():
                steps.append(self.run_step("Supabase Sync", [refresh_py, "./sync_to_supabase.py"], env))
